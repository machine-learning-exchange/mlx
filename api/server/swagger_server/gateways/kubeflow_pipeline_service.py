...
from swagger_server.data_access.minio_client import (
    ...
    _host as minio_host,
    _port as minio_port,
    _access_key as minio_access_key,
    _secret_key as minio_secret_key,
...
_host = env.get(
    "ML_PIPELINE_SERVICE_HOST", "ml-pipeline.%s.svc.cluster.local" % _namespace
)
_port = env.get("ML_PIPELINE_SERVICE_PORT", "8888")
_api_base_path = env.get("ML_PIPELINE_SERVICE_API_BASE_PATH", "")
_pipeline_service_url = env.get(
    "ML_PIPELINE_SERVICE_URL", f"{_host}:{_port}/{_api_base_path}".rstrip("/")
)


def upload_pipeline_to_kfp(
    uploadfile: str, name: str = None, description: str = None
) -> ApiPipeline:
    ...
    try:
        ...
        status_code = (
            409 if "already exist. Please specify a new name" in error_msg else e.status
        )
...
def generate_method_arg_from_parameter(parameter):
    ...
    if parameter.value or parameter.default:
        ...
        parameter.value == "" or parameter.default == ""
    ):  # TODO: should empty string != None ?
        arg = f"{param_name}=''"
    else:
        arg = param_name
...
def generate_component_run_script(
    ...
    parameter_dict = json.dumps(
        {
            sanitize(p.name): run_parameters.get(p.name) or p.default or ""
    ...
    pipeline_server = (
        "" if "POD_NAMESPACE" in os.environ else f"'{_pipeline_service_url}'"
    )
...
def generate_custom_pipeline_function_body(
    custom_pipeline: ApiPipelineCustom, hide_secrets=True
):
    ...
    for task in custom_pipeline.dag.tasks:
        ...
        if task.artifact_type == "notebook":
            ...
            task_parameters = (
                list(task.arguments.parameters)
                if task.arguments and task.arguments.parameters
                else []
            )
            ...
            for p in task_parameters:
                if type(p.value) == str and p.value.startswith("{{inputs.parameters."):
                    raise ApiError(
                        "Referencing '{{inputs.parameters.*}}' is not supported for notebook parameter"
                        f" values: {task.to_dict()}",
                        422,
                    )

            notebook_parameters = {
                p.name: p.value or p.default for p in task_parameters
            }
            notebook_parameters_str = (
                json.dumps(notebook_parameters) if notebook_parameters else ""
            )
            ...
            if missing_parameter_values:
                raise ApiError(
                    f"Missing required task parameters {missing_parameter_values}", 422
                )
            ...
            for p in task_parameters:
                ...
                if type(p.value) == str and p.value.startswith("{{inputs.parameters."):
                    match = re.match(
                        r"{{inputs.parameters.(?P<pipeline_parameter_name>\w+)}}",
                        p.value,
                    )

                    if not match:
                        raise ApiError(
                            f"Cannot match pipeline input.parameter '{p.value}'", 422
                        )

                    pipeline_param_ref = match.groupdict().get(
            ...
            raise ApiError(
                f"Unknown or unsupported artifact_type '{task.artifact_type}':\n'{task}'",
                422,
            )
    ...
    for task in custom_pipeline.dag.tasks:
        for required_task_name in task.dependencies or []:
            substitutions = {
                "op_name": "op_"
                + re.sub(r"\W+", "_", task.name, flags=re.ASCII).lower(),
                "required_op_name": "op_"
                + re.sub(r"\W+", "_", required_task_name, flags=re.ASCII).lower(),
            }
            template_rendered = Template(op_dependency_template_raw).substitute(
                substitutions
            )
            function_body += template_rendered
...
def generate_custom_pipeline_run_script(
    ...
    pipeline_method_args = generate_pipeline_method_args(
        custom_pipeline.inputs.parameters
    )

    parameter_dict = json.dumps(
        {
            p.name: run_parameters.get(p.name) or p.value or p.default  # or ""
...
def generate_dataset_run_script(
    dataset: ApiDataset,
    dataset_template_url,
    run_parameters=dict(),
    run_name: str = None,
    fail_on_missing_prereqs=False,
):
    ...
    dataset_parameters = [
        ApiParameter(name="action", default="create"),
    ...
    # see component name at https://github.com/machine-learning-exchange/mlx/blob/main/components/component-samples/dax-to-dlf/component.yaml#L1  # noqa: E501
    dax_to_dlf_component_id = generate_id(name="Generate Dataset Metadata")

    # see component name at https://github.com/machine-learning-exchange/mlx/blob/main/components/component-samples/dlf/component.yaml#L1  # noqa: E501
    dlf_to_pvc_component_id = generate_id(name="Create Dataset Volume")

    dax_to_dlf_component_url = get_object_url(
        bucket_name="mlpipeline",
        prefix=f"components/{dax_to_dlf_component_id}/",
        file_extensions=[".yaml"],
    )

    dlf_to_pvc_component_url = get_object_url(
        bucket_name="mlpipeline",
...
def generate_model_run_script(
    ...
    if (
        ...
        and model.servable_credentials_required
        or pipeline_stage == "train"
        and model.trainable_credentials_required
    ):

        template_file = (
            f"{pipeline_stage}_{execution_platform.lower()}_w_credentials.TEMPLATE.py"
        )
    else:
    ...
    substitutions = {
        ...
        "generated_secret": ""
        if hide_secrets
        else f"secret-{generate_id(length=8).lower()}",
    }
...
def generate_notebook_run_script(
    ...
    requirements_url = get_object_url(
        bucket_name="mlpipeline",
    ...
    cos_dependencies_archive_url = get_object_url(
        bucket_name="mlpipeline",
        prefix=f"notebooks/{api_notebook.id}/",
    ...
    if not cos_dependencies_archive_url:
        ...
        cos_dependencies_archive_url = store_file(
            bucket_name="mlpipeline",
            prefix=f"notebooks/{api_notebook.id}/",
    ...
    yaml_file_content = retrieve_file_content(
        bucket_name="mlpipeline",
        prefix=f"notebooks/{api_notebook.id}/",
        file_extensions=[".yaml", ".yml"],
    )
    ...
    image = metadata_yaml["implementation"]["github"].get(
...
def run_component_in_experiment(
    component: ApiComponent,
    component_template_url: str,
    parameters: dict,
    run_name: str = None,
    wait_for_status: bool = False,
):

    source_code = generate_component_run_script(
        component, component_template_url, parameters, run_name
    )

    return run_code_in_experiment(source_code, wait_for_status)


def run_custom_pipeline_in_experiment(
    custom_pipeline: ApiPipelineCustom,
    run_name: str,
    parameters: dict,
    wait_for_status: bool = False,
    ...
    try:
        source_code = generate_custom_pipeline_run_script(
            custom_pipeline, parameters, run_name, hide_secrets=False
        )

    except Exception as e:
        # TODO: remove this debug logging for development only
        print(
            f"Error trying to generate code for custom pipeline run '{run_name or custom_pipeline.name}': {e}"
        )
    ...
    try:
        ...
        print(
            f"SyntaxError trying to run pipeline DSL '{run_name or custom_pipeline.name}': {e}"
        )
        print(source_code)
        print("Custom pipeline payload:")
        print(custom_pipeline)
        raise ApiError(
            f"SyntaxError trying to run pipeline DSL: {e.msg}\n" f"{source_code}", 500
        )

    except Exception as e:
        # TODO: remove this debug logging for development only
        print(
            f"Error trying to run custom pipeline code '{run_name or custom_pipeline.name}': {e}"
        )
        print(custom_pipeline)
        print(source_code)
        raise e
...
def run_dataset_in_experiment(
    dataset: ApiDataset,
    dataset_template_url: str,
    parameters: dict = {},
    run_name: str = None,
    wait_for_status: bool = False,
):

    source_code = generate_dataset_run_script(
        dataset,
        dataset_template_url,
        parameters,
        run_name,
...
def run_model_in_experiment(
    ...
    source_code = generate_model_run_script(
        model,
        pipeline_stage,
...
def run_notebook_in_experiment(
    ...
    source_code = generate_notebook_run_script(
        notebook, parameters, run_name, hide_secrets=False
    )

    return run_code_in_experiment(source_code, wait_for_status)


def run_pipeline_in_experiment(
    ...
    try:
        ...
        if wait_for_status:
            ...
            run_status = json.loads(run_details.pipeline_runtime.workflow_manifest)[
                "status"
            ]
            ...
            if (
                ...
                raise RuntimeError(
                    f"Run {run_id} failed with error: {run_status['message']}"
                )
        ...
        print(
            f"Exception trying to run pipeline {api_pipeline.id} '{api_pipeline.name}'"
            f" with parameters {parameters}:"
            f" %s\n" % e
        )
        raise ApiError(
            message=f"{e.body}\nKFP URL: {_pipeline_service_url}",
...
def run_code_in_experiment(source_code: str, wait_for_status=False) -> str:
    ...
    try:
        ...
        print(
            "\n".join(
                [
                    "{}{:3d}: {}".format(">" if n + 1 == e.lineno else " ", n + 1, l)
