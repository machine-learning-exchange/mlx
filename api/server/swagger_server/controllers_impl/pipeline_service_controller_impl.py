...
from swagger_server.gateways.kubeflow_pipeline_service import (
    upload_pipeline_to_kfp,
    delete_kfp_pipeline,
    run_pipeline_in_experiment,
    run_custom_pipeline_in_experiment,
    _host as KFP_HOST,
)
...
def delete_pipeline(id):  # noqa: E501
    ...
    if KFP_HOST == "UNAVAILABLE":
        ...
        if id == "*":
            ...
            delete_object(
                bucket_name="mlpipeline", prefix="pipelines", file_name=f"{id}"
            )
    else:
...
def get_pipeline(id):  # noqa: E501
    ...
    """
    api_pipelines: [ApiPipelineExtended] = load_data(
        ApiPipelineExtended, filter_dict={"id": id}
    )
...
def get_template(id):  # noqa: E501
    ...
    try:
        template_yaml, url = get_file_content_and_url(
            bucket_name="mlpipeline", prefix="pipelines", file_name=id
        )
...
def list_pipelines(
    ...
    api_pipelines: [ApiPipeline] = load_data(
        ApiPipelineExtended,
    ...
    pipeline_list = ApiListPipelinesResponse(
        pipelines=api_pipelines, total_size=total_size, next_page_token=next_page_token
    )
...
def run_custom_pipeline(run_custom_pipeline_payload, run_name=None):  # noqa: E501
    ...
    duplicate_task_names = [
        name for name, count in Counter(task_names).items() if count > 1
    ]
    assert not duplicate_task_names, f"duplicate task names: {duplicate_task_names}"

    # validate pipeline dependencies
    pipeline_tasks_by_name: typing.Dict[str, ApiPipelineTask] = {
        t.name: t for t in custom_pipeline.dag.tasks
    }
    for t in pipeline_tasks_by_name.values():
        for required_task_name in t.dependencies or []:
            assert (
                required_task_name in pipeline_tasks_by_name
    ...
    missing_run_parameters = {
        ...
        if p.default is None and p.value is None
    } - run_parameters.keys()
    assert (
        not missing_run_parameters
    ), f"missing parameters to run pipeline: {missing_run_parameters}"

    # make sure we enable anonymous read access to pipeline task components
    for artifact_type in set(
        [t.artifact_type for t in pipeline_tasks_by_name.values()]
    ):
        enable_anonymous_read_access(
            bucket_name="mlpipeline", prefix=f"{artifact_type}s/*"
        )

    try:
        run_id = run_custom_pipeline_in_experiment(
            custom_pipeline, run_name, run_parameters
        )
...
def upload_pipeline(
    uploadfile, name=None, description=None, labels=None, annotations=None
):  # noqa: E501
    """upload_pipeline
    ...
    :param labels: A string representation of a JSON dictionary of labels describing this pipeline, optional.See https://kubernetes.io/docs/concepts/overview/working-with-objects/labels  # noqa: E501
    :type labels: str
    :param annotations: A string representation of a JSON dictionary of annotations describing this pipeline, optional.See https://kubernetes.io/docs/concepts/overview/working-with-objects/annotations  # noqa: E501
    ...
    return _upload_pipeline_yaml(
        yaml_file_content, name, description, labels, annotations
    )
...
def _upload_pipeline_yaml(
    ...
    try:
        ...
        if KFP_HOST == "UNAVAILABLE":
            ...
            api_pipeline: ApiPipeline = _store_pipeline(
                yaml_file_content, name, description
            )
            ...
            if not description:
                ...
                pipeline_spec = json.loads(
                    annotations.get("pipelines.kubeflow.org/pipeline_spec", "{}")
                )
                description = (
                    description or pipeline_spec.get("description", "").strip()
                )

            api_pipeline: ApiPipeline = upload_pipeline_to_kfp(
        ...
        if annotations:
            ...
            update_multiple(
                ApiPipelineExtension, [api_pipeline.id], "annotations", annotations
            )
...
def _validate_parameters(api_pipeline: ApiPipeline, parameters: dict) -> (str, int):
    ...
    if unexpected_parameters:
        return (
            f"Unexpected parameter(s): {list(unexpected_parameters)}. "
            f"Acceptable parameter(s): {acceptable_parameters}",
            422,
        )

    missing_parameters = [
        p.name
        for p in api_pipeline.parameters
