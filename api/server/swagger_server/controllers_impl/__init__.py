...
def get_yaml_file_content_from_uploadfile(uploadfile: FileStorage):
    ...
    if file_ext in ["tgz", "gz"]:
        ...
        raise ApiError(
            ...
            f"Supported file extensions: .tar.gz, .gz, .yaml, .yml",
            501,
        )
...
def validate_parameters(api_parameters: [ApiParameter], parameters: dict) -> (str, int):
    ...
    missing_parameters = [
        p.name for p in api_parameters if not p.default and p.name not in parameters
    ]
...
def validate_id(id: str) -> (str, int):
    ...
    if id != sanitize_k8s_name(id):
        return (
            f"Identifiers must contain lower case alphanumeric characters or '-' only.",
            422,
        )
...
def download_file_content_from_url(url: str, bearer_token: str = None) -> bytes:
    ...
    raise ApiError(
        f"Could not download file '{url}'. Reason: {response.reason}",
