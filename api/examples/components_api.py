...
yaml_files = glob.glob(
    "./../../../katalog/component-samples/**/component.yaml", recursive=True
)
...
def upload_component_template(uploadfile_name, name=None) -> str:
    ...
    try:
        component: ApiComponent = api_instance.upload_component(
        ...
        print(
            "Exception when calling ComponentServiceApi -> upload_component: %s\n" % e,
            file=stderr,
        )
        raise e
...
def upload_component_file(component_id, file_path):
    ...
    try:
        response = api_instance.upload_component_file(
            id=component_id, uploadfile=file_path
        )
        print(f"Upload file '{file_path}' to component with ID '{component_id}'")

    except ApiException as e:
        print(
            "Exception when calling ComponentServiceApi -> upload_component_file: %s\n"
            % e,
            file=stderr,
        )
        raise e
...
def download_component_tgz(component_id) -> str:
    ...
    try:
        response: HTTPResponse = api_instance.download_component_files(
            component_id, include_generated_code=True, _preload_content=False
        )

        attachment_header = response.info().get(
            "Content-Disposition", f"attachment; filename={component_id}.tgz"
        )
...
def verify_component_download(component_id: str) -> bool:
    ...
    try:
        ...
        generate_code_response: ApiGenerateCodeResponse = (
            api_instance.generate_component_code(component_id)
        )
...
def approve_components_for_publishing(component_ids: [str]):
    ...
    try:
        ...
        print(
            "Exception when calling ComponentServiceApi -> approve_components_for_publishing: %s\n"
            % e,
            file=stderr,
        )
...
def set_featured_components(component_ids: [str]):
    ...
    try:
        ...
        print(
            "Exception when calling ComponentServiceApi -> set_featured_components: %s\n"
            % e,
            file=stderr,
        )
...
def get_component(component_id: str) -> ApiComponent:
    ...
    try:
        ...
        print(
            "Exception when calling ComponentServiceApi -> get_component: %s\n" % e,
            file=stderr,
        )
...
def delete_component(component_id: str):
    ...
    try:
        ...
        print(
            "Exception when calling ComponentServiceApi -> delete_component: %s\n" % e,
            file=stderr,
        )
...
def generate_code(component_id: str) -> str:
    ...
    try:
        ...
        print(
            "Exception while calling ComponentServiceApi -> generate_code: %s\n" % e,
            file=stderr,
        )
...
def run_code(component_id: str, parameters: dict = {}, run_name: str = None) -> str:
    ...
    try:
        ...
        run_code_response: ApiRunCodeResponse = api_instance.run_component(
            component_id, param_array, run_name=run_name
        )
        ...
        print(
            "Exception while calling ComponentServiceApi -> run_code: %s\n" % e,
            file=stderr,
        )
...
def main():
    ...
    component = list_components(
        filter_dict={"name": "Create Secret - Kubernetes Cluster"}
    )[0]
