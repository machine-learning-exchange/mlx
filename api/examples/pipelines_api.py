...
yaml_files = glob.glob(
    "./../../../kfp-tekton/sdk/python/tests/compiler/testdata/*.yaml"
)[:10]
...
def upload_pipeline_template(
    uploadfile_name, name: str = None, description: str = None, annotations: str = ""
) -> str:
...
def upload_pipeline_file(pipeline_id, file_path):
    ...
    try:
        response = api_instance.upload_pipeline_file(
            id=pipeline_id, uploadfile=file_path
        )
        print(f"Upload file '{file_path}' to pipeline with ID '{pipeline_id}'")

    except ApiException as e:
        print(
            "Exception when calling PipelineServiceApi -> upload_pipeline_file: %s\n"
            % e,
            file=stderr,
        )
        raise e
...
def approve_pipelines_for_publishing(pipeline_ids: [str]):
    ...
    try:
        ...
        print(
            "Exception when calling PipelineServiceApi -> approve_pipelines_for_publishing: %s\n"
            % e,
            file=stderr,
        )
...
def set_featured_pipelines(pipeline_ids: [str]):
    ...
    try:
        ...
        print(
            "Exception when calling PipelineServiceApi -> set_featured_pipelines: %s\n"
            % e,
            file=stderr,
        )
...
def get_pipeline(pipeline_id: str) -> ApiPipelineExtended:
    ...
    try:
        ...
        print(
            "Exception when calling PipelineServiceApi -> get_pipeline: %s\n" % e,
            file=stderr,
        )
...
def delete_pipeline(pipeline_id: str):
    ...
    try:
        ...
        print(
            "Exception when calling PipelineServiceApi -> delete_pipeline: %s\n" % e,
            file=stderr,
        )
...
@print_function_name_decorator
def run_pipeline(
    pipeline_id: str, parameters: dict = None, run_name: str = None
) -> str:
    ...
    try:
        run_code_response: ApiRunCodeResponse = api_instance.run_pipeline(
            pipeline_id, parameters=parameters, run_name=run_name
        )
        ...
        print(
            "Exception while calling PipelineServiceApi -> run_pipeline: %s\n" % e,
            file=stderr,
        )
...
def generate_custom_pipeline(sequential_only=False) -> ApiPipelineCustom:
    ...
    try:
        components: typing.List[ApiComponent] = random.sample(
            list(
                filter(
                    lambda n: "StudyJob" not in n.name,
            ...
            for p in t.arguments.parameters:
                if not (p.value or p.default):
                    ...
                    pipeline_params.append(
                        ApiParameter(name=pipeline_param_name, value="some value")
                    )
...
def run_custom_pipeline(
    pipeline_template: dict, parameters: list = None, run_name: str = None
) -> str:
    ...
    try:
        ...
        if pipeline_template.get("spec", {}).get("templates"):
            ...
            pipeline_dag = list(
                filter(lambda t: "dag" in t, pipeline_template["spec"]["templates"])
            )[0]
        ...
        custom_pipeline = api_client.deserialize(
            response=mock_response, response_type=ApiPipelineCustom
        )
        run_custom_pipeline_payload = ApiPipelineCustomRunPayload(
            custom_pipeline=custom_pipeline, run_parameters=parameters
        )
        run_code_response: ApiRunCodeResponse = api_instance.run_custom_pipeline(
            run_custom_pipeline_payload=run_custom_pipeline_payload, run_name=run_name
        )
        print(run_code_response.run_url)
        return run_code_response.run_url
...
def main():
    ...
    pipeline_id = upload_pipeline_template(
        tarfile_name, name, description, annotations_str
    )
    ...
    pipelines = (
        list_pipelines(filter_dict={"name": "[Sample] Basic - Parallel execution"})
        or list_pipelines(filter_dict={"name": "[Sample] Basic - Parallel Join"})
        or list_pipelines(filter_dict={"name": "[Tutorial] DSL - Control structures"})
        or list_pipelines(filter_dict={"name": "test_parallel_join"})
    )
