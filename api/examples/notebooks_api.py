...
from swagger_client.models import ApiNotebook, ApiGetTemplateResponse, ApiListNotebooksResponse, \
...
def upload_notebook_template(uploadfile_name, name=None, ghe_token=None) -> str:
    ...
    try:
        if ghe_token:
            notebook: ApiNotebook = api_instance.upload_notebook(uploadfile=uploadfile_name, name=name,
                                                                 enterprise_github_token=ghe_token)
        else:
            notebook: ApiNotebook = api_instance.upload_notebook(uploadfile=uploadfile_name, name=name)
...
def upload_notebook_file(notebook_id, file_path):
    ...
    try:
        response = api_instance.upload_notebook_file(id=notebook_id, uploadfile=file_path)
...
@print_function_name_decorator
def download_notebook_tgz(notebook_id,
                          download_dir=os.path.join(tempfile.gettempdir(), "download", "notebooks")) -> str:
    ...
    try:
        ...
        attachment_header = response.info().get("Content-Disposition",
                                                f"attachment; filename={notebook_id}.tgz")
...
def verify_notebook_download(notebook_id: str) -> bool:
    ...
    try:
        ...
        template_response: ApiGetTemplateResponse = api_instance.get_notebook_template(notebook_id)
        ...
        generate_code_response: ApiGenerateCodeResponse = api_instance.generate_notebook_code(notebook_id)
...
def get_template(template_id: str) -> str:
    ...
    try:
        template_response: ApiGetTemplateResponse = api_instance.get_notebook_template(template_id)
...
def generate_code(notebook_id: str) -> str:
    ...
    try:
        ...
        print("Exception while calling NotebookServiceApi -> generate_code: %s\n" % e, file=stderr)
...
def run_notebook(notebook_id: str, parameters=dict(), run_name: str = None) -> str:
    ...
    try:
        run_code_response: ApiRunCodeResponse = api_instance.run_notebook(notebook_id,
        ...
        print("Exception while calling NotebookServiceApi -> run_code: %s\n" % e, file=stderr)
...
def list_notebooks(filter_dict: dict = {}, sort_by: str = None) -> [ApiNotebook]:
    ...
    try:
        ...
        api_response: ApiListNotebooksResponse = api_instance.list_notebooks(filter=filter_str, sort_by=sort_by)
