...
def deploy_split_rule(primary_model_revision, new_revision, traffic_percentage, traffic_route_name, namespace='default'):
    ...
    namespace = spec["metadata"]["namespace"]               # TODO: the namespace should be configured or be figured out dynamically
...
def deploy_single_rule(revision, traffic_route_name, namespace='default'):
...
def update_knative_spec(params):
    ...
    if "submission_id" in params:
        env_dict["SUBMISSION_ID"] = params['submission_id']
...
def delete_deployment(params):
    ...
    spec = get_knative_spec(params)
    spec["metadata"]["name"] = params["deployment_name"]
    spec["metadata"]["namespace"] = params["namespace"] if "namespace" in params else "default"
    name = spec["metadata"]["name"]
...
def get_pods(params):
    ...
    namespace = params['namespace'] if 'namespace' in params else 'default'
    pods = api_client_v1.list_namespaced_pod(namespace=namespace, watch=False)
...
def rule_api_post():
    if not request.json:
        abort(400)
    namespace = request.json['namespace'] if 'namespace' in request.json else 'default'
    return json.dumps(deploy_split_rule(request.json['primary_model_revision'], request.json['deployment_revision'], request.json['traffic_percentage'], request.json['traffic_route_name'], namespace=namespace))
...
def single_rule_api_post():
    if not request.json:
        abort(400)
    namespace = request.json['namespace'] if 'namespace' in request.json else 'default'
    return json.dumps(deploy_single_rule(request.json['revision'], request.json['traffic_route_name'], namespace=namespace))
