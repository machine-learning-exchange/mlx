...
function KFServingCard(props: KFServingCardProps) {
  const {
    name, statusIcon, link, tag, framework, asset, classes,
  } = props;
        ...
        >
          <Grid item>
            <div style={{ display: 'flex', flexDirection: 'row' }}>
              <CardHeader
                title={name.replace(/(^|-)[a-z]/g, (a) => (a[1] ? ` ${a[1]}` : a[0]).toUpperCase())}
          ...
          <Grid item>
            <Grid container justify="space-between">
              <Grid item>
                <CardContent className={classes.wrap}>
                  {tags.map((tag) => <Chip key={tag} label={tag} variant="outlined" className={classes.chip} />)}
                </CardContent>
              </Grid>
              {logo
                && (
