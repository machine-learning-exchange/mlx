...
export class SettingsPage extends React.Component<ISettingsPageProps, any> {
  ...
  apply = (name: string, value: string) => {
    ...
    if (name === 'API Endpoint') API = value;
  ...
  handleToggle = (name: string) => async (event: any) => this.apply(name, event.currentTarget.checked);

  handleSelect = (name: string) => async (event: any) => this.apply(name, event.currentTarget.value);
  ...
  reset = async () => {
    ...
    const changes = Object.values(this.context.store.settings).map((section) => Object.values(section).map((setting) => [setting.name, setting.default])).flat();
  ...
  public render() {
    const { store } = this.context;
    const {
      branding, kfserving, endpoints, capabilities, artifacts,
    } = store.settings;
