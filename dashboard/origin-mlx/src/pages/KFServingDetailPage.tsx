...
async function fetchAssetTemplates(API: string, type: string, asset: any) {
  ...
  if (Array.isArray(data)) {
    if (!typesWithMultipleTemplates.includes(type)) throw Error(`Received multiple templates for <${type}>. Expected one.`);
        ...
        .then((template: any) => {
          if (props.asset.search) {
            ...
            url_params.forEach((param: string) => {
              ...
              if (type === 'pipelines') url_params_json[split[0].replace(/_/g, '-')] = split[1];
              else url_params_json[split[0]] = split[1];
            });
          ...
          ? (
            <div className="detail-body-wrapper">
              ...
              {React.cloneElement(child, {
                namespace, type, id, ...child.props, asset,
              })}
              {' '}
            </div>
          )
