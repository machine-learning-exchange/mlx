...
export default class KFServingDetail extends React.Component<KFServingDetailProps, KFServingDetailState> {
  ...
  getCode = (platform: string) => this.state.service.code
  ...
  public render() {
    ...
    if (service.status?.condition) {
      for (let conditionIter = 0; conditionIter < service.status.conditions.length; conditionIter += 1) {
        const condition = service.status.conditions[conditionIter];
      ...
      for (let i = 0; i < service.status.conditions.length; i += 1) {
    ...
    return (
      <>
            ...
            >
              ...
              { false
                ...
                <Popup
                  trigger={(
                    <button style={{
                      paddingTop: '10px', paddingBottom: '10px', border: 'none', outline: 'none', fontSize: '14px', marginTop: '20px', backgroundColor: '#1BCDC7', color: 'white', width: '620px',
                    }}
                    >
                      {' '}
                      <img
                        src={updateIcon}
                        style={{
                          width: '15px', height: 'auto', marginBottom: '-3px', marginRight: '8px',
                        }}
                        alt=""
                      />
                      UPDATE SERVICE
                  ...
                  position="top center"
                  contentStyle={{
                    marginTop: '30px', marginLeft: '300px', width: '1165px', height: '670px', zIndex: 100, opacity: 1, boxShadow: '0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19)', borderWidth: '5px', borderColor: '#1BCDC7',
                  }}
                  arrow={false}
                >
        ...
        { this.state.topTab === 'kiali'
          && <iframe title="kiali" className="service-iframe" src={kialiLink} />}
        { this.state.topTab === 'grafana'
          && <iframe title="grafana" className="service-iframe" src={grafanaLink} />}
        { this.state.topTab === 'graph'
          && <iframe title="graph" className="service-iframe" src={graphLink} />}
      </>
    );
  }
}
