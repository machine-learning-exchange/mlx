...
export default class ModelRunForm extends React.Component<ModelRunFormProps, any> {
  constructor(props:any) {
    ...
    const shownParams: { [key: string]: { description: string, value: string } } = {};
    ...
    if (props.inputParameters) {
      for (let yamlParamIter = 0; yamlParamIter < props.inputParameters.length; yamlParamIter += 1) {
        let hasMatch = false;
        const curParam = props.inputParameters[yamlParamIter];
        Object.keys(props.urlParameters).forEach((key: string) => {
          if (key === curParam.name) hasMatch = true;
        });
        // If there is no matching url_parameter add the parameter to the form
        if (!hasMatch) shownParams[curParam.name] = { description: curParam.description, value: curParam.default };
        else matches.push(curParam.name);
      }
    }
    ...
    Object.keys(props.urlParameters).forEach((key: string) => {
      matches.forEach((match: string) => {
        if (key === match) matchedUrlParams[key] = props.urlParameters[key];
      });
    });
    this.state = {
      isLoading: false,
  ...
  getRunTypes = () => {
    const types = [];
    if (this.props.trainingPlatforms.length) { types.push('training'); }
    if (this.props.servingPlatforms.length) { types.push('serving'); }
    return types;
  };

  getPlatforms = (runType:string) => {
    if (runType === 'training') { return this.props.trainingPlatforms.filter((platform) => platform !== 'knative'); }
    if (runType === 'serving') { return this.props.servingPlatforms.filter((platform) => platform !== 'knative'); }
  };
              ...
              <div>
                ...
                <form autoComplete="off">
                  <TextField
                    ...
                    name="runType"
                  >
                    {this.getRunTypes().map((name: string, i: number) => <MenuItem key={i} value={name}>{ capitalized(name) }</MenuItem>)}
                  </TextField>
                  <TextField
                    select
                    SelectProps={{ MenuProps: { MenuListProps: { style: { width: '100%' } } } }}
                    ...
                    name="platform"
                  >
                    { this.getPlatforms(this.state.runType).map((name:string, i:number) => <MenuItem key={i} value={name}>{ name.length > 3 ? capitalized(name) : name.toUpperCase() }</MenuItem>)}
                  </TextField>
                  <TextField
                    autoCorrect="false"
                    id="runName"
