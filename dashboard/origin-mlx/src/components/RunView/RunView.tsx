...
function RunView(props: RunViewProps) {
  ...
  const setRunLink = props.setRunLink

  var shownParams: {[key: string]: any}[] = []
  ...
  if (asset.parameters) {
    for(var assetIter = 0; assetIter < asset.parameters.length; assetIter++) {
      let hasMatch = false
      for(const key of Object.keys(asset.url_parameters)) {
        if (key === asset.parameters[assetIter].name)
          hasMatch = true
      }
      ...
      if (!hasMatch)
        ...
        matches.push(asset.parameters[assetIter].name)
    }
  }
  ...
  Object.keys(asset.url_parameters).forEach((key: string) => {
    matches.forEach((match: string) => {
      if (key === match)
        matchedUrlParams[key] = asset.url_parameters[key]
    })
  })
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    var payload: RunParameters = {name: parameters.name}
              ...
              { type === "operator" ?
                <TextField
                  ...
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                      setRun({ ...run, name: e.currentTarget.value })}
                /> :
                <TextField
                  autoCorrect="false"
                  // id='runName'
                  ...
                  style={{ backgroundColor: 'blue !important', }}
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                      setRun({ ...run, runname: e.currentTarget.value })}
                />
              }
              { type === "datasets" &&
                <TextField
                  ...
                  InputLabelProps={{ shrink: true, }}
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    setRun({ ...run, namespace: e.currentTarget.value })}
                />
              }
              { type === "notebooks" &&
                <>
                  <TextField
                    ...
                    InputLabelProps={{ shrink: true, }}
                    onChange={(e: ChangeEvent<HTMLInputElement>) =>
                      setRun({ ...run, dataset_pvc: e.currentTarget.value })}
                  />
                  <TextField
                    key="mount-path"
                    label='Mount Path'
                    ...
                    InputLabelProps={{ shrink: true, }}
                    onChange={(e: ChangeEvent<HTMLInputElement>) =>
                      setRun({ ...run, mount_path: e.currentTarget.value })}
                  />
                </>
              }
