...
export default class PipelineDetailGraph extends React.Component<IPipelineDetailGraphProps, any> {
  ...
  inRect = (mouseX: number, mouseY: number, rect: any) => (mouseX >= rect.x && mouseX <= rect.x + rect.width) && (mouseY >= rect.y && mouseY <= rect.y + rect.height);
  ...
  buildGraph = (entrypoint: string, templates: any[]) => {
    const dagNodes = this.props.templates.filter((t:any) => t.hasOwnProperty('dag'));

    const nonDagNodes = this.props.templates.filter((t:any) => !t.hasOwnProperty('dag')).map((node) => node.name);
